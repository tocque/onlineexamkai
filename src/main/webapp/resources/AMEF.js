...
(function () {
                ...
                case 0:
                    {
                        if (ch === "{") {
                            ...
                            while(script[next-1] === '\\' && script[next-2] !== '\\') {
                                next = script.indexOf(ch, next + 1);
                            }
    ...
    var loadedCount = 0, taskCount = 0; window.__taskstat__ = new Set();
    var Module = (function () {
        function Module(hook, path) {
            taskCount++;
            this.hook = hook;
            __taskstat__.add(path);
            hook.then(() => { 
                loadedCount++; 
                __taskstat__.delete(path);
                console.log(`module ${path} loaded: [${loadedCount}/${taskCount}]`)
            });
        }
                        ...
                        return __generator(this, function (_a) {
                            switch (_a.label) {
                                case 0:
                                    ...
                                    const telm = sandbox.content.querySelector("template");
                                    if (telm) {
                                        template = telm.innerHTML;
                                    }
                                    styles = sandbox.content.querySelectorAll("style");
                                    if (url == "/OnlineExam/resources/layout/index.vue") {
                                        console.log(`=================>${styles}<=================`);
                                    }
    ...
    var _import = function (path, now) {
        if (now === void 0) { now = window.location.pathname; }
        return __awaiter(this, void 0, void 0, function () {
            var suffix;
            return __generator(this, function (_a) {
                ...
                if (!(path in cache)) {
                    cache[path] = new Module(loaders[suffix](path), path);
                }
