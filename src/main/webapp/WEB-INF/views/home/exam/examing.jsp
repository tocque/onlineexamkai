        ...
        <div class="nr_left">
            <div class="test">
                <form action="" method="post">
                    ...
                    <c:if test="${ exam.multiQuestionNum !=0 }">
                        <div class="test_content">
                            <div class="test_content_title">
                                <h2>多选题</h2>
                                <p>
                                    <span>共</span><i
                                        class="content_lit">${ exam.multiQuestionNum }</i><span>题，</span><span>合计</span><i
                                        class="content_fs">${ exam.multiQuestionNum * multiScore}</i><span>分</span>
                                </p>
                            </div>
                        </div>
                        <div class="test_content_nr">
                            <ul>
                                <c:forEach items="${ multiQuestionList}" var="mq" varStatus="mqids">
                                    <li id="qu_${ multiQuestion }_${ mq.question.id}" data-key="${ mq.id}">
                                        <div class="test_content_nr_tt">
                                            <i>${ mqids.index + exam.singleQuestionNum + 1 }</i><span>(${ mq.question.score}分)</span><font>${ mq.question.title }</font>
                                        </div>
                                        <div class="test_content_nr_main">
                                            <ul>
                                                <li class="option" data-type="multi" data-value="A">
                                                    <input type="checkbox" class="radioOrCheck" name="answer1"
                                                           id="${ multiQuestion }_answer_${ mq.id}_option_1" value="A"
                                                    />
                                                    <label for="${ multiQuestion }_answer_${ mq.id}_option_1">
                                                        A.
                                                        <p class="ue" style="display: inline;">${ mq.question.attrA}</p>
                                                    </label>
                                                </li>
                                                <li class="option" data-type="multi" data-value="B">
                                                    <input type="checkbox" class="radioOrCheck" name="answer1"
                                                           id="${ multiQuestion }_answer_${ mq.id}_option_2" value="B"
                                                    />
                                                    <label for="${ multiQuestion }_answer_${ mq.id}_option_2">
                                                        B.
                                                        <p class="ue" style="display: inline;">${ mq.question.attrB}</p>
                                                    </label>
                                                </li>
                                                <li class="option" data-type="multi" data-value="C">
                                                    <input type="checkbox" class="radioOrCheck" name="answer1"
                                                           id="${ multiQuestion }_answer_${ mq.id}_option_3" value="C"
                                                    />
                                                    <label for="${ multiQuestion }_answer_${ mq.id}_option_3">
                                                        C.
                                                        <p class="ue" style="display: inline;">${ mq.question.attrC}</p>
                                                    </label>
                                                </li>
                                                <li class="option" data-type="multi" data-value="D">
                                                    <input type="checkbox" class="radioOrCheck" name="answer1"
                                                           id="${ multiQuestion }_answer_${ mq.id}_option_4" value="D"
                                                    />
                                                    <label for="${ multiQuestion }_answer_${ mq.id}_option_4">
                                                        D.
                            ...
                            <ul>
                                <c:forEach items="${ chargeQuestionList}" var="cq" varStatus="cqids">
                                    <li id="qu_${ chargeQuestion }_${ cq.question.id}" data-key="${ cq.id}">
                                        <div class="test_content_nr_tt">
                                            <i>${ cqids.index+exam.singleQuestionNum+exam.multiQuestionNum+1}</i><span>(${ cq.question.score}分)</span><font>${ cq.question.title }</font>
                                        </div>
                                        <div class="test_content_nr_main">
                                            <ul>
                    ...
                    <c:if test="${ exam.multiQuestionNum !=0 }">
                        <div class="rt_content">
                            <div class="rt_content_tt">
                                <h2>多选题</h2>
                                <p>
                                    <span>共</span><i class="content_lit">${ exam.multiQuestionNum }</i><span>题</span>
                                </p>
                            </div>
                            <div class="rt_content_nr answerSheet">
                                <ul>
                                    <c:forEach items="${ multiQuestionList}" var="mmq" varStatus="mmqids">
                                        <li>
                                            <a href="#qu_${ multiQuestion }_${ mmq.question.id }">${ mqids.index + exam.singleQuestionNum + 1 }</a>
                                        </li>
                                    </c:forEach>
                                </ul>
                            </div>
                            ...
                            <div class="rt_content_nr answerSheet">
                                <ul>
                                    <c:forEach items="${ chargeQuestionList}" var="ccq" varStatus="ccqids">
                                        <li>
                                            <a href="#qu_${ chargeQuestion}_${ ccq.question.id}">${ ccqids.index+exam.singleQuestionNum+exam.multiQuestionNum+1}</a>
                                        </li>
                                    </c:forEach>
                                </ul>
                            </div>
...
<script>
    ...
    $(function () {
        $('li.option input').click(function () {
            ...
            if ($(this).closest("li.option").attr("data-type") == 'multi') {
                //如果是多选
                selectedAnswer = '';
